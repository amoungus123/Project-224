...
def Bike_control();
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.56))
    time.sleep(15)
    dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))

    dropped_vehicle.apply_control(Carla.VehicleControl(throttle=0.25, steer=-0.35))
    dropped_vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.HighBeam | carla.VehicleLightState.Brake | carla.VehicleLightState.LeftBeam))
...
try:
    ...
    car_model = get_blueprint_of_world.filter('name of the vehicle')[0]
